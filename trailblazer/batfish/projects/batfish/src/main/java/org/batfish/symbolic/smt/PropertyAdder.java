...
class PropertyAdder {
   ...
   */
  private List<BoolExpr> initializeReachabilityVars(
      EncoderSlice slice,
      Context ctx,
      Solver solver,
    ...
    //TODO: test
    List<BoolExpr> boundExprs = new ArrayList<>();
    ...
    for (String r : _encoderSlice.getGraph().getRouters()) {
      ...
      //TODO: test
      boundExprs.add(ctx.mkEq(var, ctx.mkGt(idVar, zero)));
      boundExprs.add(ctx.mkGe(idVar, zero));
    }
    return boundExprs;
  }
  ...
  Tuple<Map<String, BoolExpr>, List<BoolExpr>> instrumentReachability(Set<GraphEdge> ges, boolean retExpr) {
    Context ctx = _encoderSlice.getCtx();
    Solver solver = _encoderSlice.getSolver();
    EncoderSlice slice = _encoderSlice;
    Map<String, BoolExpr> reachableVars = new HashMap<>();
    Map<String, ArithExpr> idVars = new HashMap<>();
    List<BoolExpr> exprs = initializeReachabilityVars(slice, ctx, solver, reachableVars, idVars);
    Graph g = _encoderSlice.getGraph();
    ...
    return new Tuple<>(reachableVars, exprs);
  }
