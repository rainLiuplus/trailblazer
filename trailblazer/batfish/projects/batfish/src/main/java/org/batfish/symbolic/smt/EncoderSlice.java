...
class EncoderSlice {
  ...
  EncoderSlice(Encoder enc, HeaderSpace h, Graph graph, String sliceName) {
    ...
    //TODO: test
    _auxiliaryLogicalGraph = _logicalGraph;
    _inactivatedEdges = new HashSet<>();
...
//      String router, Configuration conf, Protocol proto) {
//    Set<LogicalEdge> eList = new HashSet<>();
//    List<ArrayList<LogicalEdge>> les = _auxiliaryLogicalGraph.getLogicalEdges().get(router, proto);
//    assert (les != null);
//    for (ArrayList<LogicalEdge> es : les) {
//      for (LogicalEdge le : es) {
//        if (_auxiliaryLogicalGraph.isEdgeUsed(conf, proto, le) && le.getEdgeType() == EdgeType.IMPORT && !_inactivatedEdges.contains(le.getEdge())) {
//          eList.add(le);
//        }
//      }
//    }
...
//  }
  ...
  private void addImportConstraint(
    ...
    BoolExpr failed = getSymbolicFailures().getFailedVariable(e.getEdge());
    assert (failed != null);
    BoolExpr notFailed = mkEq(failed, mkBool(false));
...
//    ArithExpr failed = getSymbolicFailures().getFailedVariable(e.getEdge());
  ...
  void computeEncoding(LogicalGraph graph, Set<GraphEdge> inactivatedEdges) {
    this.setAuxiliaryLogicalGraph(graph);
    this.setInactivatedEdges(inactivatedEdges);
  ...
  public void setAuxiliaryLogicalGraph(LogicalGraph graph){
    _logicalGraph = graph;
  }

  public void setInactivatedEdges(Set<GraphEdge> edges) {
    _inactivatedEdges = edges;
  }
}
