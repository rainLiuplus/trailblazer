...
public class Encoder {
  ...
  private LogicalGraph _auxiliaryLogicalGraph;

  private Set<GraphEdge> _inactivatedEdges;
  ...
  private Encoder(
    ...
    if (solver == null) {
      if (ENABLE_UNSAT_CORE) {
        ...
        Tactic t6 = _ctx.mkTactic("card2bv");
        ...
        Tactic t = _ctx.then(t1,  t2, t6, t3, t4, t5);
    ...
    for (List<GraphEdge> edges : _graph.getEdgeMap().values()) {
      for (GraphEdge ge : edges) {
        if (ge.getPeer() == null) {
          ...
          BoolExpr var = _ctx.mkBoolConst(name);
  ...
  BoolExpr mkBoolConst(String name){
    return getCtx().mkBoolConst(name);
  }
  ...
  //Symbolic PbLe of expressions
  BoolExpr PbLe(int[] coeffs, BoolExpr[] args, int k){
    return getCtx().mkPBEq(coeffs, args, k);
  }

  BoolExpr PbEq(int[] coeffs, BoolExpr[] args, int k){
    return getCtx().mkPBEq(coeffs, args, k);
  }

  BoolExpr PbGe(int[] coeffs, BoolExpr[] args, int k){
    return getCtx().mkPBGe(coeffs, args, k);
  }
  ...
  public void addRecursivePTConstraints(String bestPathWithBrokenEdges){
    ...
    for(String derivedBestPathPair: derivedBestPathPairs){
      ...
      BoolExpr oneAssign = mkTrue();
      for(String brokenEdge: brokenEdges){
        String router = brokenEdge.split("=")[0];
        String peer = brokenEdge.split("=")[1];
        String pair = (router.compareTo(peer) < 0 ? router + "-" + peer : peer + "-" + router);
        BoolExpr var = getSymbolicFailures().getFailedInternalLinks().get(pair.split("-")[0], pair.split("-")[1]);
        oneAssign = mkAnd(oneAssign, mkEq(var, mkTrue()));
      }
      add(mkImplies(oneAssign, getPTConstraints(derivedBestPath)));
    }
  }

  public BoolExpr getPTConstraints(String bestEdges){
    if (bestEdges.equals(""))
      return mkTrue();
    List<BoolExpr> pVars = new ArrayList<>();
    String [] newHotEdges = bestEdges.split(",");
    ...
    int[] array = new int[pVars.size()];
    Arrays.fill(array, 1);

    BoolExpr[] nVars = new BoolExpr[pVars.size()];
    List<BoolExpr> nt = pVars.stream().collect(Collectors.toList());
    for (int i=0; i < pVars.size(); i++) {
      nVars[i] = nt.get(i);
    }
    return PbGe(array, nVars, 1);
  }

  public void addPTConstraints(int k, String fullLinkPaths){
    if (k==0 || fullLinkPaths.equals(""))
      return;
    List<BoolExpr> pVars = new ArrayList<>();
    for (String fullLinkPath: fullLinkPaths.split(";")){
      String [] pathNodes = fullLinkPath.split("-");
      for (int i = 0; i < pathNodes.length - 1; i++){
        String router = pathNodes[i];
        String peer = pathNodes[i+1];
        String pair = (router.compareTo(peer) < 0 ? router + "-" + peer : peer + "-" + router);
        BoolExpr var = getSymbolicFailures().getFailedInternalLinks().get(pair.split("-")[0], pair.split("-")[1]);
        pVars.add(var);
      }
    ...
    add(PbGe(array, nVars, 1));
  }

  public Tuple<BoolExpr, BoolExpr> addIncrvFailedConstraints(int k, Set<BoolExpr> vars){
      int[] array = new int[vars.size()];
      Arrays.fill(array, 1);

      BoolExpr[] nVars = new BoolExpr[vars.size()];
      List<BoolExpr> nt = vars.stream().collect(Collectors.toList());
      for (int i=0; i < vars.size(); i++) {
        nVars[i] = nt.get(i);
      }

      return new Tuple<>(PbGe(array, nVars, k), PbEq(array, nVars, k));
  }


  public BoolExpr getFailedConstraints(int k, Set<BoolExpr> vars){
    int[] array = new int[vars.size()];
    Arrays.fill(array, 1);
    BoolExpr[] nVars = new BoolExpr[vars.size()];
    List<BoolExpr> nt = new ArrayList<>(vars);

    for (int i=0; i < vars.size(); i++) {
      nVars[i] = nt.get(i);
    }
    return PbEq(array, nVars , k);
  }

  public void addFailedConstraints(int k, String fullLinkPaths, Set<BoolExpr> vars){
    ...
    if (fullLinkPaths.equals("")){
      return;
    }
  }


  /* Generate constraints for link failures with aware of fullLinkPath*/
  private void addFailedLinkConstraints(int k, String fullLinkPath){
    Set<BoolExpr> vars = new HashSet<>();
    getSymbolicFailures().getFailedInternalLinks().forEach((router, peer, var) -> vars.add(var));
    addFailedConstraints(k, fullLinkPath, vars);
  }
  ...
  public void setAuxiliaryLogicalGraph(LogicalGraph graph) {this._auxiliaryLogicalGraph = graph;}

  public void setInactivatedEdges(Set<GraphEdge> edges) {this._inactivatedEdges = edges;}
}
